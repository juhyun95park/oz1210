...
> **중요**: 2025년 4월 1일부터 Clerk의 JWT 템플릿 방식은 deprecated되었습니다. 이제 **네이티브 Supabase 통합**을 사용합니다. 이 방식은 더 간단하고 안전하며, Supabase JWT secret을 Clerk와 공유할 필요가 없습니다.

**3-1. Clerk Dashboard에서 Supabase 통합 활성화**

1. [Clerk Dashboard](https://dashboard.clerk.com/)에 로그인
2. 프로젝트 선택 → **Setup** → **Integrations** 메뉴로 이동
3. **"Supabase"** 통합 찾기
4. **"Activate Supabase integration"** 클릭
5. 통합이 활성화되면 **Clerk domain**이 표시됩니다 (예: `your-app-12.clerk.accounts.dev`)
   - 이 Clerk domain을 복사해두세요 (다음 단계에서 사용)

> **참고**: 통합을 활성화하면 Clerk의 모든 세션 토큰에 `"role": "authenticated"` 클레임이 자동으로 추가됩니다. 이는 Supabase가 사용자를 인증된 사용자로 인식하는 데 필요합니다.
...
4. **"Add provider"** 클릭
5. 제공자 목록에서 **"Clerk"** 선택
6. **"Clerk domain"** 필드에 3-1에서 복사한 Clerk domain 입력
   - 예: `your-app-12.clerk.accounts.dev`
7. **"Add provider"** 또는 **"Save"** 클릭

**3-3. 통합 확인**

통합이 완료되면:
- Clerk에서 로그인한 사용자의 세션 토큰이 Supabase에서 자동으로 인식됩니다
- RLS 정책에서 `auth.jwt()->>'sub'`를 사용하여 Clerk user ID를 확인할 수 있습니다
- Supabase 클라이언트에 Clerk 세션 토큰을 전달하면 자동으로 인증됩니다

**통합 테스트:**
1. 개발 서버 실행: `pnpm dev`
2. 브라우저에서 `/auth-test` 페이지 접속
3. Clerk로 로그인
4. "Supabase 연결 상태"가 "연결 성공!"으로 표시되는지 확인
5. "Supabase Users 테이블 데이터" 섹션에 사용자 정보가 표시되는지 확인
...
> **참고**: 개발 단계에서는 RLS가 비활성화되어 있습니다. 프로덕션 배포 전에 적절한 RLS 정책을 설정하세요.

**예제 테이블 생성 (선택사항):**

Supabase 공식 문서의 예제처럼 `instruments` 테이블을 생성하여 테스트할 수 있습니다:
