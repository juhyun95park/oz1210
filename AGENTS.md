...
이 프로젝트는 Clerk와 Supabase의 **네이티브 통합** (2025년 4월 1일 이후 권장 방식)을 사용합니다:

> **중요**: JWT 템플릿 방식은 deprecated되었습니다. 네이티브 통합은 더 간단하고 안전하며, Supabase JWT secret을 Clerk와 공유할 필요가 없습니다.

#### 설정 요구사항

1. **Clerk Dashboard에서 Supabase 통합 활성화**:
   - Setup → Integrations → Supabase → "Activate Supabase integration"
   - 활성화 시 Clerk domain이 생성됨 (예: `your-app-12.clerk.accounts.dev`)
   - 통합 활성화 후 모든 Clerk 세션 토큰에 `"role": "authenticated"` 클레임이 자동 추가됨

2. **Supabase Dashboard에서 Clerk를 Third-Party Auth Provider로 추가**:
   - Authentication → Sign In / Up → Third-Party Auth → Add provider → Clerk
   - Clerk domain 입력 (위에서 생성된 domain)
   - 이 설정으로 Supabase가 Clerk 세션 토큰을 자동으로 검증

#### 인증 흐름

1. **사용자 인증**: Clerk가 사용자 인증 처리
2. **사용자 동기화**: `SyncUserProvider`가 로그인 시 자동으로 Clerk 사용자를 Supabase `users` 테이블에 동기화
3. **데이터 접근**: Supabase 클라이언트가 Clerk 세션 토큰을 사용하여 인증
   - RLS 정책에서 `auth.jwt()->>'sub'`로 Clerk user ID 확인
   - `auth.uid()`는 사용 불가 (Clerk는 문자열 ID, Supabase는 UUID 사용)
...
- **`clerk-client.ts`**: Client Component용 (`useClerkSupabaseClient` hook)
  ...
  - `accessToken` 옵션으로 Supabase 클라이언트에 토큰 전달
  - RLS 정책이 `auth.jwt()->>'sub'`로 Clerk user ID 확인
  - Clerk 통합이 필요한 경우 사용

- **`server.ts`**: Server Component/Server Action용 (`createClerkSupabaseClient`)
  - `auth().getToken()`으로 서버 사이드에서 Clerk 세션 토큰 획득
  - `accessToken` 옵션으로 Supabase 클라이언트에 토큰 전달
  - Clerk 통합이 필요한 경우 사용

**표준 Supabase 클라이언트 (Supabase Auth 또는 공개 데이터용):**

- **`server-standard.ts`**: Server Component/Server Action용 (`createClient`)
  - `@supabase/ssr` 패키지의 `createServerClient` 사용
  - Cookie 기반 세션 관리 (Supabase Auth 사용 시)
  - Supabase 공식 문서의 권장 패턴
  - 공개 데이터 접근 또는 Supabase Auth 사용 시 사용

- **`client.ts`**: Client Component용 (표준 브라우저 클라이언트)
...
- **`app/api/sync-user/route.ts`**: 실제 동기화 로직 (API 라우트)
  ...
  - Clerk 사용자 정보를 Supabase `users` 테이블에 upsert
